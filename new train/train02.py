...
New features:
 ...
 - build_sankey(): generates Sankey diagrams showing motif → antigen (epitope gene) → CDR3 → epitope
...
class TCRPredictor:
    ...
    ########################################
    # 新增：Sankey (motif -> antigen(epitope gene) -> CDR3 -> epitope)
    ########################################
    def build_sankey(self, df, output_dir="results", motif_size=3, max_gap=1,
                     top_k_motifs=20, top_k_antigens=10, top_k_cdr3_per_antigen=5):
        """
        Build a Sankey diagram connecting:
            motif -> antigen (Epitope gene) -> CDR3 sequence -> Epitope (epitope string)
        Saves:
            - results/sankey.html (plotly interactive) if plotly available
            - results/sankey.png (static) if possible
        Parameters limited to avoid enormous graphs:
            top_k_motifs: how many top motifs to include
            top_k_antigens: how many top antigen genes to include
            top_k_cdr3_per_antigen: per antigen, how many representative CDR3 to include
        """
        os.makedirs(output_dir, exist_ok=True)

        # basic checks
        if 'CDR3' not in df.columns:
            logging.error("DataFrame missing 'CDR3' column for Sankey.")
            return None
        if 'Epitope gene' not in df.columns:
            logging.warning("DataFrame missing 'Epitope gene' column; filling with 'UNKNOWN'.")
            df['Epitope gene'] = df.get('Epitope gene', pd.Series(['UNKNOWN'] * len(df)))

        sequences = df['CDR3'].astype(str).tolist()
        # build top motifs
        top_motifs = build_topk_motif_list(sequences, motif_size=motif_size, max_gap=max_gap, topk=top_k_motifs)
        if len(top_motifs) == 0:
            logging.warning("No motifs found for Sankey.")
            return None
        M = motif_presence_matrix(sequences, top_motifs)  # (n_seq, n_motifs)

        # antigen list (Epitope gene)
        df['Epitope gene'] = df['Epitope gene'].fillna('UNKNOWN').astype(str)
        antigen_counts = df['Epitope gene'].value_counts()
        top_antigens = antigen_counts.head(top_k_antigens).index.tolist()
        if len(top_antigens) == 0:
            logging.warning("No antigen (Epitope gene) data for Sankey.")
            return None

        # we'll build node lists and link counts
        motif_nodes = [f"MOTIF:{m}" for m in top_motifs]
        antigen_nodes = [f"ANTIGEN:{a}" for a in top_antigens]

        # choose representative CDR3s: for each antigen, choose top_k_cdr3_per_antigen sequences (if duplicates exist)
        cdr3_nodes = []
        cdr3_set = set()
        antigen_to_cdr3s = {}
        for ag in top_antigens:
            subset = df[df['Epitope gene'] == ag]
            # choose most frequent CDR3s for that antigen (usually unique)
            top_cdr3s = subset['CDR3'].value_counts().head(top_k_cdr3_per_antigen).index.tolist()
            antigen_to_cdr3s[ag] = top_cdr3s
            for c in top_cdr3s:
                if c not in cdr3_set:
                    cdr3_set.add(c)
                    cdr3_nodes.append(f"CDR3:{c}")

        # epitope nodes: map chosen CDR3s to their epitope strings
        epitope_nodes = []
        cdr3_to_epitope = {}
        for c in list(cdr3_set):
            rows = df[df['CDR3'] == c]
            if not rows.empty:
                epi = rows.iloc[0].get('Epitope', 'UNKNOWN')
                if not isinstance(epi, str):
                    epi = str(epi)
            else:
                epi = 'UNKNOWN'
            cdr3_to_epitope[c] = epi
            if epi not in epitope_nodes:
                epitope_nodes.append(epi)

        # build unified node list and index mapping
        node_labels = []
        node_labels.extend(motif_nodes)
        node_labels.extend(antigen_nodes)
        node_labels.extend(cdr3_nodes)
        node_labels.extend(epitope_nodes)
        node_to_idx = {lbl: i for i, lbl in enumerate(node_labels)}

        sources = []
        targets = []
        values = []

        # motif -> antigen (count of sequences under antigen that contain motif)
        for mi, motif in enumerate(top_motifs):
            motif_label = f"MOTIF:{motif}"
            motif_idx = node_to_idx.get(motif_label)
            # find sequences with motif
            seq_indices = np.where(M[:, mi] == 1)[0]
            if seq_indices.size == 0:
                continue
            # count antigens among these sequences
            sub_antigens = df.iloc[seq_indices]['Epitope gene'].fillna('UNKNOWN').astype(str)
            counts = sub_antigens.value_counts()
            for ag, cnt in counts.items():
                if ag not in top_antigens:
                    continue
                antig_label = f"ANTIGEN:{ag}"
                antig_idx = node_to_idx.get(antig_label)
                if antig_idx is None or motif_idx is None:
                    continue
                sources.append(motif_idx)
                targets.append(antig_idx)
                values.append(int(cnt))

        # antigen -> CDR3 (connect representative cdr3s)
        for ag in top_antigens:
            antig_label = f"ANTIGEN:{ag}"
            if antig_label not in node_to_idx:
                continue
            antig_idx = node_to_idx[antig_label]
            for c in antigen_to_cdr3s.get(ag, []):
                c_label = f"CDR3:{c}"
                if c_label not in node_to_idx:
                    continue
                c_idx = node_to_idx[c_label]
                # weight: number of occurrences of that CDR3 for that antigen (usually 1)
                wt = int(len(df[(df['CDR3'] == c) & (df['Epitope gene'] == ag)]))
                if wt <= 0:
                    wt = 1
                sources.append(antig_idx)
                targets.append(c_idx)
                values.append(wt)

        # CDR3 -> Epitope (connect each chosen cdr3 to its epitope)
        for c in cdr3_set:
            c_label = f"CDR3:{c}"
            c_idx = node_to_idx.get(c_label)
            epi = cdr3_to_epitope.get(c, 'UNKNOWN')
            if epi not in epitope_nodes:
                continue
            epi_idx = node_to_idx.get(epi)
            if c_idx is None or epi_idx is None:
                continue
            sources.append(c_idx)
            targets.append(epi_idx)
            values.append(int(len(df[df['CDR3'] == c])))  # frequency of this cdr3 overall

        # if no links -> bail
        if len(values) == 0:
            logging.warning("No links available to render Sankey (after filtering).")
            return None
        ...
        try:
            ...
            fig.update_layout(title_text="Sankey: motif -> antigen(epitope gene) -> CDR3 -> epitope", font_size=10, height=900)
            html_path = os.path.join(output_dir, "sankey.html")
            fig.write_html(html_path)
            logging.info(f"Sankey interactive saved to {html_path}")

            # attempt to save static PNG (requires kaleido or orca); if unavailable ignore
            try:
                png_path = os.path.join(output_dir, "sankey.png")
                fig.write_image(png_path)
                logging.info(f"Sankey static image saved to {png_path}")
            except Exception as e:
                logging.info(f"Could not save Sankey PNG via plotly (kaleido/orca missing?): {e}")

            return {'html': html_path, 'png': os.path.join(output_dir, "sankey.png") if os.path.exists(os.path.join(output_dir, "sankey.png")) else None}
        except Exception as e_plotly:
            logging.warning(f"plotly not available or failed ({e_plotly}). Falling back to static network drawing using networkx/matplotlib.")

        # Fallback: draw a layered directed graph using networkx + matplotlib
        try:
            Gs = nx.DiGraph()
            for src, tgt, val in zip(sources, targets, values):
                s_label = node_labels[src]
                t_label = node_labels[tgt]
                if Gs.has_edge(s_label, t_label):
                    Gs[s_label][t_label]['weight'] += val
                else:
                    Gs.add_edge(s_label, t_label, weight=val)

            # layout nodes in 4 columns: motifs, antigens, cdr3s, epitopes
            pos = {}
            # compute y coord spacing for each column
            def place_column(items, x):
                ys = np.linspace(0, 1, max(1, len(items)+1))[1:len(items)+1] if len(items) > 0 else []
                for i, it in enumerate(items):
                    pos[it] = (x, 1.0 - (i / max(1, len(items)-1)) if len(items) > 1 else 0.5)

            place_column(motif_nodes, 0.0)
            place_column(antigen_nodes, 0.33)
            place_column(cdr3_nodes, 0.66)
            place_column(epitope_nodes, 1.0)

            plt.figure(figsize=(18, 8))
            # draw nodes
            nx.draw_networkx_nodes(Gs, pos, node_size=400, node_color='lightblue')
            # draw labels (shorten CDR3 labels for readability)
            labels = {}
            for n in Gs.nodes():
                if n.startswith("CDR3:"):
                    labels[n] = n.replace("CDR3:", "")[:12]
                elif n.startswith("MOTIF:"):
                    labels[n] = n.replace("MOTIF:", "")
                elif n.startswith("ANTIGEN:"):
                    labels[n] = n.replace("ANTIGEN:", "")
                else:
                    labels[n] = n[:20]
            nx.draw_networkx_labels(Gs, pos, labels, font_size=8)

            # draw edges with widths proportional to weight
            all_weights = [d['weight'] for (u, v, d) in Gs.edges(data=True)]
            max_w = max(all_weights) if all_weights else 1
            for (u, v, d) in Gs.edges(data=True):
                nx.draw_networkx_edges(Gs, pos, edgelist=[(u, v)],
                                       width=max(0.5, 3.0 * (d['weight'] / max_w)),
                                       alpha=0.7, arrows=True, arrowstyle='-|>', arrowsize=12)

            plt.title("Sankey-like flow (motif -> antigen -> CDR3 -> epitope) [static fallback]", fontsize=14)
            plt.axis('off')
            png_path = os.path.join(output_dir, "sankey_fallback.png")
            plt.savefig(png_path, dpi=200, bbox_inches='tight')
            plt.close()
            logging.info(f"Sankey fallback saved to {png_path}")
            return {'png': png_path}
        except Exception as e_net:
            logging.error(f"Failed to create Sankey fallback: {e_net}")
            return None

    # rest of methods already defined above (predict, cluster_analysis, visualize_clusters, etc.)
    # ... (they are present above)
...
if __name__ == "__main__":
    try:
        ...
        # Build Sankey: motif -> antigen(epitope gene) -> CDR3 -> epitope
        try:
            sankey_res = predictor.build_sankey(df, output_dir="results", motif_size=3, max_gap=1,
                                                top_k_motifs=30, top_k_antigens=15, top_k_cdr3_per_antigen=6)
            logging.info(f"Sankey build result: {sankey_res}")
        except Exception as e:
